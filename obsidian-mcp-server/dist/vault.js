...
export class ObsidianVault {
    ...
    async getAllMarkdownFiles(dir = this.vaultPath) {
    ...
    getFileType(relativePath) {
        if (relativePath.includes('Periodic Notes'))
            return 'daily';
        if (relativePath.includes('Projects'))
            return 'project';
        if (relativePath.includes('Reference'))
            return 'reference';
        if (relativePath.includes('Transcripts/Limitless'))
            return 'transcript';
        return 'other';
    }
    setupFileWatcher() {
    ...
    async search(query, type = 'all', limit = 10) {
        ...
        const filtered = type === 'all'
            ? results
            : results.filter(r => {
                const metadataPath = r.item.metadata?.path;
        ...
        for (const result of filtered.slice(0, limit)) {
            const metadataPath = result.item.metadata?.path;
            const metadataTitle = result.item.metadata?.title;
            const metadataType = result.item.metadata?.type;
    ...
    async getDailyContext(date, options) {
        const targetDate = date || new Date().toISOString().split('T')[0];
        const context = { date: targetDate };
        // Get daily note
    ...
    async getRecentFiles(folder, days) {
